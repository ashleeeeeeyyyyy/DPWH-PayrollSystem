...
<html>
	...
	<body>
		<div id="sub">
			<br><h2 style="text-align:center";>Employee Detail</h2><br>
			...
			$result = $conn->query("SELECT * FROM employees WHERE id ='".$id."'");
				...
				while($row2 = $result->fetch_assoc()){
					...
					echo "<tr>
							...
							<th>CAREA</th>
							<th>DIS-ALOWNCE / LWOP</th>
							<th>Total Deductions</th>
							<th>Total Amount Earned</th>
							<th colspan ='2'>Action</th>
						</tr>";

					$result2 = $conn->query("SELECT * FROM payroll_data LEFT JOIN employees ON payroll_data.id = employees.id WHERE employees.id ='".$row2['id']."' AND payroll_data.year = '".$year."' ORDER BY payroll_data.month_num ASC");
					...
					while($row = $result2->fetch_assoc()){
							...
							$var1  = $conn->query("SELECT SUM(pera) AS total FROM payroll_data WHERE id = '{$row['id']}' AND year = '{$row['year']}'");
							$var2  = $conn->query("SELECT SUM(monthly_salary) as total FROM payroll_data WHERE id = '{$row['id']}' AND year = '{$row['year']}'");
							$var3  = $conn->query("SELECT SUM(gross_amount_earned) as total FROM payroll_data WHERE id = '{$row['id']}' AND year = '{$row['year']}'");
							$var4  = $conn->query("SELECT SUM(taxwh) as total FROM payroll_data WHERE id = '{$row['id']}' AND year = '{$row['year']}'");
							$var5  = $conn->query("SELECT SUM(gsis_cont) as total FROM payroll_data WHERE id = '{$row['id']}' AND year = '{$row['year']}'");
							$var6  = $conn->query("SELECT SUM(phic_cont) as total FROM payroll_data WHERE id = '{$row['id']}' AND year = '{$row['year']}'");
							$var7  = $conn->query("SELECT SUM(hdmf_cont) as total FROM payroll_data WHERE id = '{$row['id']}' AND year = '{$row['year']}'");
							$var8  = $conn->query("SELECT SUM(hdmf_mp) as total FROM payroll_data WHERE id = '{$row['id']}' AND year = '{$row['year']}'");
							$var9  = $conn->query("SELECT SUM(hdmf_cal) as total FROM payroll_data WHERE id = '{$row['id']}' AND year = '{$row['year']}'");
							$var10 = $conn->query("SELECT SUM(hdmf_loan) as total FROM payroll_data WHERE id = '{$row['id']}' AND year = '{$row['year']}'");
							$var11 = $conn->query("SELECT SUM(hdmf_hsg) as total FROM payroll_data WHERE id = '{$row['id']}' AND year = '{$row['year']}'");
							$var12 = $conn->query("SELECT SUM(cons) as total FROM payroll_data WHERE id = '{$row['id']}' AND year = '{$row['year']}'");
							$var13 = $conn->query("SELECT SUM(emr) as total FROM payroll_data WHERE id = '{$row['id']}' AND year = '{$row['year']}'");
							$var14 = $conn->query("SELECT SUM(eduass_loan) as total FROM payroll_data WHERE id = '{$row['id']}' AND year = '{$row['year']}'");
							$var15 = $conn->query("SELECT SUM(plreg) as total FROM payroll_data WHERE id = '{$row['id']}' AND year = '{$row['year']}'");
							$var16 = $conn->query("SELECT SUM(plopt) as total FROM payroll_data WHERE id = '{$row['id']}' AND year = '{$row['year']}'");
							$var17 = $conn->query("SELECT SUM(opt_life) as total FROM payroll_data WHERE id = '{$row['id']}' AND year = '{$row['year']}'");
							$var18 = $conn->query("SELECT SUM(rel) as total FROM payroll_data WHERE id = '{$row['id']}' AND year = '{$row['year']}'");
							$var19 = $conn->query("SELECT SUM(philam) as total FROM payroll_data WHERE id = '{$row['id']}' AND year = '{$row['year']}'");
							$var20 = $conn->query("SELECT SUM(caremco_cap) as total FROM payroll_data WHERE id = '{$row['id']}' AND year = '{$row['year']}'");
							$var21 = $conn->query("SELECT SUM(caremco_loan) as total FROM payroll_data WHERE id = '{$row['id']}' AND year = '{$row['year']}'");
							$var22 = $conn->query("SELECT SUM(caremco_canteen) as total FROM payroll_data WHERE id = '{$row['id']}' AND year = '{$row['year']}'");
							$var23 = $conn->query("SELECT SUM(allowance) as total FROM payroll_data WHERE id = '{$row['id']}' AND year = '{$row['year']}'");
							$var24 = $conn->query("SELECT SUM(total_deduct) as total FROM payroll_data WHERE id = '{$row['id']}' AND year = '{$row['year']}'");
							$var25 = $conn->query("SELECT SUM(first_q) as total FROM payroll_data WHERE id = '{$row['id']}' AND year = '{$row['year']}'");
							$var26 = $conn->query("SELECT SUM(carea) as total FROM payroll_data WHERE id = '{$row['id']}' AND year = '{$row['year']}'");
							...
							echo '<td><a href ="edit_payroll_details_month.php?id='.$row['id'].'&year='.$row['year'].'&month='.$row['month_num'].'">Edit</a></td>';
							echo '<td><a href ="data_delete.php?id='.$row['id'].'&year='.$row['year'].'&month='.$row['month_num'].'">Delete</a></td>';
							//echo '<td><input class = "tb_size2" readonly type="text" value="'.$format_var26.'"</input></td>';
